...
var (
	...
	ethPortFlag = flag.Int("ethport", 60909, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the TarCoin protocol")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for TarCoin and the faucet")
)
...
type request struct {
	...
	Account common.Address     `json:"account"` // TarCoin address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an TarCoin light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // TarCoin protocol stack
	client *trcnclient.Client  // Client connection to the TarCoin chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the TarCoin light client protocol
...
// close terminates the TarCoin connection and tears down the faucet.
...
func (f *faucet) apiHandler(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		// Retrieve the TarCoin address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
...
// the username, avatar URL and TarCoin address to fund on success.
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the TarCoin address and profile URL.
	...
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", common.Address{}, errors.New("No TarCoin address found to fund")
	}
	var avatar string
...
// returning the username, avatar URL and TarCoin address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain TarCoin address,
