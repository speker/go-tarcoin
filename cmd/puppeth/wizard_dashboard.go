...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"trcnstats", "explorer", "wallet", "faucet"} {
	...
	// If we have trcnstats running, ask whether to make the secret public or not
	if w.conf.ethstats != "" {
		fmt.Println()
		fmt.Println("Include trcnstats secret on dashboard (y/n)? (default = yes)")
