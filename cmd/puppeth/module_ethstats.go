...
// deployEthstats deploys a new trcnstats container to a remote machine via SSH,
...
func deployEthstats(client *sshClient, network string, port int, secret string, vhost string, trusted []string, banned []string, nocache bool) ([]byte, error) {
	...
	// Build and deploy the trcnstats service
	if nocache {
...
// ethstatsInfos is returned from an trcnstats status check to allow reporting
...
// checkEthstats does a health-check against an trcnstats server to verify whether
// it's running, and if yes, gathering a collection of useful infos about it.
func checkEthstats(client *sshClient, network string) (*ethstatsInfos, error) {
	// Inspect a possible trcnstats container on the host
