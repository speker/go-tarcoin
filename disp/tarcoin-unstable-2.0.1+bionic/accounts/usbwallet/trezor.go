...
import (
	...
	"github.com/speker/go-tarcoin/accounts/usbwallet/trezor"
...
// or whether the TarCoin app was not started on it.
...
// TarCoin address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.TarCoinAddress)
	if _, err := w.trezorExchange(&trezor.TarCoinGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.TarCoinTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.TarCoinTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
