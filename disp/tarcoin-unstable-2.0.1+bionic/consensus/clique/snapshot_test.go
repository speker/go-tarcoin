...
// mapped from textual names used in the tests below to actual TarCoin private
...
// address retrieves the TarCoin address of a tester account by label, creating
...
func (ap *testerAccountPool) address(account string) common.Address {
	...
	// Resolve and return the TarCoin address
...
func TestClique(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			// batch (https://github.com/speker/go-tarcoin/issues/17593). Whilst this
	...
	for i, tt := range tests {
		...
		for j, block := range blocks {
			// Gtrcn the header and prepare it for signing
			header := block.Header()
			if j > 0 {
