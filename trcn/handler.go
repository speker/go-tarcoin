...
// NewProtocolManager returns a new TarCoin sub protocol manager. The TarCoin sub protocol manages peers capable
// with the TarCoin network.
func NewProtocolManager(config *params.ChainConfig, checkpoint *params.TrustedCheckpoint, mode downloader.SyncMode, networkID uint64, mux *event.TypeMux, txpool txPool, engine consensus.Engine, blockchain *core.BlockChain, chaindb trcndb.Database, cacheLimit int, whitelist map[uint64]common.Hash) (*ProtocolManager, error) {
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing TarCoin peer", "peer", id)

	// Unregister the peer from the downloader and TarCoin peer set
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("TarCoin protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("TarCoin peer connected", "name", p.Name())

	// Execute the TarCoin handshake
	var (
	...
	if err := p.Handshake(pm.networkID, td, hash, genesis.Hash(), forkid.NewID(pm.blockchain), pm.forkFilter); err != nil {
		p.Log().Debug("TarCoin handshake failed", "err", err)
		return err
	}

	// Register the peer locally
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("TarCoin peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("TarCoin message handling failed", "err", err)
			return err
		}
	}
}
...
// NodeInfo represents a short summary of the TarCoin sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // TarCoin network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
