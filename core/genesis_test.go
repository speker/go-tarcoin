...
func TestSetupGenesis(t *testing.T) {
	var (
		customghash = common.HexToHash("0x8da729462fd39d1347da127b8cfa6b6054da38661be78748546de4ed3b3e3e9f")
		customg     = Genesis{
	...
	tests := []struct {
		name       string
		fn         func(trcndb.Database) (*params.ChainConfig, common.Hash, error)
	...
	}{
		{
			...
			fn: func(db trcndb.Database) (*params.ChainConfig, common.Hash, error) {
				return SetupGenesisBlock(db, new(Genesis))
			},
			wantErr:    errGenesisNoConfig,
			wantConfig: params.AllTrcnhashProtocolChanges,
		},
		{
		...
		},
		{
			...
			fn: func(db trcndb.Database) (*params.ChainConfig, common.Hash, error) {
				...
				bc, _ := NewBlockChain(db, nil, oldcustomg.Config, trcnhash.NewFullFaker(), vm.Config{}, nil, nil)
				defer bc.Stop()

				blocks, _ := GenerateChain(oldcustomg.Config, genesis, trcnhash.NewFaker(), db, 4, nil)
