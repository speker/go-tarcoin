...
// https://raw.githubusercontent.com/LedgerHQ/blue-app-trcn/master/doc/ethapp.asc
...
const (
	ledgerOpRetrieveAddress  ledgerOpcode = 0x02 // Returns the public key and TarCoin address for a given BIP 32 path
	ledgerOpSignTransaction  ledgerOpcode = 0x04 // Signs an TarCoin transaction after having the user validate the parameters
...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "TarCoin app in browser mode", w.failure
	}
	if w.offline() {
		return "TarCoin app offline", w.failure
	}
	return fmt.Sprintf("TarCoin app v%d.%d.%d online", w.version[0], w.version[1], w.version[2]), w.failure
}

// offline returns whether the wallet and the TarCoin app is offline or not.
//
...
func (w *ledgerDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	if err != nil {
		// TarCoin app is not running or in browser mode, nothing more to do, return
		if err == errLedgerReplyInvalidHeader {
			w.browser = true
		}
		return nil
	}
	// Try to resolve the TarCoin app's version, will fail prior to v1.0.2
...
// and returning the TarCoin address located on that derivation path.
...
func (w *ledgerDriver) SignTx(path accounts.DerivationPath, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	// If the TarCoin app doesn't run, abort
	if w.offline() {
...
// ledgerVersion retrieves the current version of the TarCoin wallet app running
...
// ledgerDerive retrieves the currently active TarCoin address from a Ledger
...
//   TarCoin address length | 1 byte
//   TarCoin address        | 40 bytes hex ascii
...
func (w *ledgerDriver) ledgerDerive(derivationPath []uint32) (common.Address, error) {
	...
	// Extract the TarCoin hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return common.Address{}, errors.New("reply lacks address entry")
	}
	hexstr := reply[1 : 1+int(reply[0])]

	// Decode the hex sting into an TarCoin address and return
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	// Extract the TarCoin signature and do a sanity validation
