...
func TestClientWebsocketPing(t *testing.T) {
	...
	// This is important for reproducing https://github.com/speker/go-tarcoin/issues/19798
	<-ctx.Done()
	close(sendPing)
...
func wsPingTestHandler(t *testing.T, conn *websocket.Conn, shutdown, sendPing <-chan struct{}) {
	// Canned responses for the trcn_subscribe call in TestClientWebsocketPing.
	const (
		subResp   = `{"jsonrpc":"2.0","id":1,"result":"0x00"}`
		subNotify = `{"jsonrpc":"2.0","method":"trcn_subscription","params":{"subscription":"0x00","result":1}}`
	)
