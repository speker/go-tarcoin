...
func TestAccountListEmpty(t *testing.T) {
	gtrcn := runGeth(t, "account", "list")
	gtrcn.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gtrcn := runGeth(t, "account", "list", "--datadir", datadir)
	defer gtrcn.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	gtrcn := runGeth(t, "account", "new", "--lightkdf")
	defer gtrcn.ExpectExit()
	gtrcn.Expect(`
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	gtrcn := runGeth(t, "account", "import", keyfile, "-password", passwordFile)
	defer gtrcn.ExpectExit()
	gtrcn.Expect(expected)
}
...
func TestAccountUpdate(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gtrcn := runGeth(t, "account", "update",
...
func TestWalletImport(t *testing.T) {
	gtrcn := runGeth(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gtrcn.ExpectExit()
	gtrcn.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(gtrcn.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gtrcn.StderrText(), m) {
...
// https://github.com/speker/go-tarcoin/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gtrcn.SetTemplateFunc("keypath", func(file string) string {
