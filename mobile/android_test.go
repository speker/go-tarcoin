...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gtrcn.LightScryptN, Gtrcn.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.tarcoin", new NodeConfig());
			node.start();
			...
			TarCoinClient ec = node.getTarCoinClient();
	...
	// signatures too. Regression test for go-tarcoin issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "gtrcn-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/tarcoin/gethtest", "libs"} {
	...
	cp.CopyFile(filepath.Join("libs", "gtrcn.aar"), "gtrcn.aar")

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "tarcoin", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
