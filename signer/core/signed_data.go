...
func (api *SignerAPI) determineSignatureFormat(ctx context.Context, contentType string, addr common.MixedcaseAddress, data interface{}) (*SignDataRequest, bool, error) {
	var (
		req          *SignDataRequest
		useTarCoinV = true // Default to use V = 27 or 28, the legacy TarCoin format
	)
	mediaType, _, err := mime.ParseMediaType(contentType)
	if err != nil {
		return nil, useTarCoinV, err
	}
	...
	case ApplicationClique.Mime:
		...
		if !ok {
			return nil, useTarCoinV, fmt.Errorf("input for %v must be an hex-encoded string", ApplicationClique.Mime)
		}
	...
	default: // also case TextPlain.Mime:
		...
		if stringData, ok := data.(string); !ok {
			return nil, useTarCoinV, fmt.Errorf("input for text/plain must be an hex-encoded string")
		} else {
